...
function CodeSnip({ code }) {
  ...
  return (
    <div className="relative rounded-xl overflow-hidden border border-white/[0.06] bg-[#050311] mt-3">
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/[0.04] bg-white/[0.01]">
        ...
        <button onClick={() => { navigator.clipboard.writeText(code); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
          className={cn('flex items-center gap-1.5 text-xs px-2 py-1 rounded transition-colors', copied ? 'text-teal-400' : 'text-white/30 hover:text-white')}>
    ...
    <div className="min-h-screen bg-[#0a0814] text-white px-6 py-12">
      <div className="max-w-2xl mx-auto">
        ...
        <BlurFade delay={0} inView className="mb-10">
          ...
          <p className="text-white/50 leading-relaxed" style={{ fontSize: 15 }}>
        ...
        <BlurFade delay={0.05} inView className="grid grid-cols-3 gap-3 mb-8">
          {[
            ...
            <div key={label} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-center">
              ...
              <div className="text-white font-bold text-xs mb-0.5" style={{ fontFamily: 'var(--font-display)' }}>{label}</div>
              ...
              <div key={s} className="flex items-center flex-1">
                <div className="flex flex-col items-center gap-1.5">
                  <div className={cn(
                    ...
                    step > i ? 'bg-purple-500 border-purple-500 text-white' :
                  ...
                  <span className={cn('text-[10px] font-semibold whitespace-nowrap', step === i ? 'text-purple-400' : step > i ? 'text-white/60' : 'text-white/20')}>{s}</span>
                </div>
                {i < STEPS.length - 1 && (
                  <div className={cn('flex-1 h-px mx-2 mb-4 transition-all duration-300', step > i ? 'bg-purple-500' : 'bg-white/[0.06]')} />
                )}
              </div>
            ))}
          </div>
        ...
        {step === 0 && (
          <BlurFade delay={0.1} inView>
            <div className="relative rounded-2xl border border-white/[0.07] bg-white/[0.02] p-7 overflow-hidden">
              ...
              <h2 className="font-bold text-white mb-2" style={{ fontFamily: 'var(--font-display)', fontSize: 20, letterSpacing: '-0.02em' }}>
              ...
              <div className="mb-5">
                <label className="block text-white/50 text-xs font-bold uppercase tracking-wider mb-2">Wallet Set Name</label>
                <input
                  value={walletSetName}
                  onChange={e => setWalletSetName(e.target.value)}
                  placeholder="e.g. AgentBoard Agents"
                  className="w-full px-4 py-3 rounded-xl border border-white/[0.08] bg-white/[0.03] text-white placeholder-white/20 text-sm outline-none focus:border-purple-500/40 transition-all"
              ...
              <button onClick={handleCreateWalletSet} disabled={loading}
                className={cn('w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-semibold text-sm text-white transition-all', loading ? 'opacity-60 cursor-not-allowed' : 'hover:scale-[1.01]')}
        ...
        {step === 1 && (
          <BlurFade delay={0.1} inView>
            <div className="relative rounded-2xl border border-white/[0.07] bg-white/[0.02] p-7 overflow-hidden">
              ...
              <h2 className="font-bold text-white mb-2 mt-1" style={{ fontFamily: 'var(--font-display)', fontSize: 20, letterSpacing: '-0.02em' }}>
              ...
              <div className="mb-5">
                <label className="block text-white/50 text-xs font-bold uppercase tracking-wider mb-2">Agent Name</label>
                <input
                  value={agentName}
        ...
        {step === 2 && wallet && (
          <BlurFade delay={0.1} inView>
            <div className="relative rounded-2xl border border-teal-500/20 bg-teal-500/[0.03] p-7 overflow-hidden mb-5">
              <BorderBeam size={200} duration={12} colorFrom="#19fb9b" colorTo="#9945ff" />
              ...
              <div className="space-y-3 mb-6">
                {[
                  ...
                  <div key={label} className="flex items-start justify-between gap-3 p-3.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                    <div>
                      <div className="text-white/30 text-[10px] font-bold uppercase tracking-wider mb-1">{label}</div>
                      <div className={cn('text-white text-sm break-all', mono && 'font-mono')} style={mono ? { fontFamily: 'var(--font-mono)' } : {}}>
                        {value}
                      </div>
                    </div>
                    {copyable && (
                      <button onClick={copyAddress}
                        className={cn('flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border transition-all shrink-0 mt-4', copiedAddr ? 'text-teal-400 border-teal-500/30 bg-teal-500/10' : 'text-white/30 border-white/10 hover:text-white')}>
            ...
            <BlurFade delay={0.15} inView>
              <div className="relative rounded-2xl border border-white/[0.07] bg-white/[0.02] p-6 overflow-hidden">
                ...
                <p className="text-white/35 text-xs mb-1">Your wallet ID is pre-filled. Copy and run in your agent server.</p>
            ...
            <div className="flex gap-3 mt-5 flex-wrap">
              <button onClick={() => navigate('/register')}
                className="flex items-center gap-2 px-5 py-3 rounded-xl font-semibold text-sm text-white transition-all hover:scale-[1.01]"
        ...
        {step < 2 && (
          <BlurFade delay={0.15} inView className="mt-8">
            <div className="rounded-2xl border border-white/[0.05] bg-white/[0.01] p-6">
              <h3 className="font-bold text-white/70 text-sm mb-4" style={{ fontFamily: 'var(--font-display)' }}>How Circle Dev-Controlled Wallets work</h3>
              <div className="space-y-4">
                {[
                  ...
                  <div key={title} className="flex items-start gap-3">
                    <div className="w-7 h-7 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center shrink-0 mt-0.5">{icon}</div>
                    <div>
