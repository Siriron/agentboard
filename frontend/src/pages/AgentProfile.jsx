    ...
    <div className="min-h-screen bg-[#0a0814] text-white px-6 py-12">
      <div className="max-w-4xl mx-auto">
        <button onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-white/40 text-sm hover:text-white transition-colors mb-8">
        ...
        {loading ? (
          <div className="flex items-center justify-center gap-4 py-24">
            ...
            <span className="text-white/40 text-sm">Loading agent profile…</span>
          </div>
        ) : (
          <>
            {/* Profile Header */}
            <BlurFade delay={0} inView>
              <div className="relative rounded-2xl border border-white/[0.07] bg-white/[0.02] p-7 mb-6 overflow-hidden">
                <BorderBeam size={250} duration={15} colorFrom="#9945ff" colorTo="#19fb9b" />
                ...
                {/* Stats row */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-7 pt-6 border-t border-white/[0.05]">
                  {[
                    ...
                    { label: 'Avg Payout', value: null, text: `$${formatUSDC(avgBudget)}`, color: 'text-white' },
                  ...
                  <div className="flex flex-col gap-3">
                    {inProgress.map(({ id, core, meta }) => (
                      <div key={id} onClick={() => navigate(`/job/${id}`)}
                        className="flex items-center justify-between gap-4 p-3 rounded-xl border border-white/[0.05] bg-white/[0.02] cursor-pointer hover:border-white/10 transition-all">
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-white text-sm truncate" style={{ fontFamily: 'var(--font-display)' }}>{meta.title}</p>
                          <p className="text-white/30 text-xs mt-0.5">#{id} · {STATUS_LABEL[Number(core.status)]}</p>
                        </div>
            ...
            <BlurFade delay={0.1} inView>
              <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
                <div className="px-6 py-4 border-b border-white/[0.05]">
                  <h2 className="font-bold text-white" style={{ fontFamily: 'var(--font-display)', fontSize: 16, letterSpacing: '-0.02em' }}>
                ...
                {jobs.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-center px-6">
                    ...
                    <p className="text-white/30 text-sm">No jobs yet for this agent.</p>
                  </div>
                ) : (
                  <div className="divide-y divide-white/[0.04]">
                    {jobs.map(({ id, core, meta }) => {
                      ...
                      return (
                        <div key={id} onClick={() => navigate(`/job/${id}`)}
                          className="flex items-center gap-4 px-6 py-4 cursor-pointer hover:bg-white/[0.02] transition-all group">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                              <span className={cn('text-[9px] font-black px-2 py-0.5 rounded border', STATUS_COLORS[sn])}>
                                {STATUS_LABEL[sn]}
                              </span>
                              <span className="text-white/20 text-xs" style={{ fontFamily: 'var(--font-mono)' }}>#{id}</span>
                            </div>
                            ...
                            <p className="text-white/30 text-xs mt-0.5">{meta.category} · Due {formatDate(core.deadline)}</p>
                          </div>
                          <div className="text-right shrink-0">
                            <div className={cn('font-bold text-sm', sn === 3 ? 'text-teal-400' : 'text-white/60')}
                              style={{ fontFamily: 'var(--font-display)', letterSpacing: '-0.02em' }}>
                              {sn === 3 ? '+' : ''}${formatUSDC(sn === 3 ? core.budget * 0.99 : core.budget)}
                            </div>
                            <div className="text-white/20 text-xs">{sn === 3 ? 'earned' : 'budget'}</div>
                          </div>
                          <ExternalLink size={13} className="text-white/15 group-hover:text-purple-400 transition-colors shrink-0" />
                        </div>
                      )
                    })}
                  </div>
