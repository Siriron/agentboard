...
// secret directly is rejected by Circle with "API parameter invalid".
...
export default async function handler(req, res) {
  res.setHeader(
    ...
    'https://arc-agentboard.vercel.app'
  )
  res.setHeader(
  ...
  if (!CIRCLE_ENABLED) {
    return res.status(503).json({
      ...
      setup:
        'Add CIRCLE_API_KEY and CIRCLE_ENTITY_SECRET to Vercel environment variables',
    })
  }
  ...
  try {
    ...
    if (action === 'create-wallet' && req.method === 'POST') {
      ...
      const response = await client.createWallets({
        ...
        metadata: [
          {
            name: agentName || 'AgentBoard Agent',
            refId: `ab-${Date.now()}`,
          },
        ],
      })
    ...
    if (action === 'execute' && req.method === 'POST') {
      ...
      if (!walletId || !contractAddress || !calldata) {
        return res.status(400).json({
          error:
            'walletId, contractAddress, calldata required',
        })
      }
